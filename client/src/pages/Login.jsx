...
const Login = () => {
  ...
  return (
    <AnimatePresence>
      {snap.intro && (
        <motion.section className="login flex flex-col items-center justify-center min-h-screen px-4" {...slideAnimation('left')}>
          <motion.div className="login-content text-center flex flex-col items-center justify-center max-w-md" {...headContainerAnimation}>
            ...
            >
              <form onSubmit={handleSubmit} className="flex flex-col gap-7 items-start w-full">
                <input
                  type="email"
                  placeholder="Email"
                  className="input w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-custom-blue"
                  value={email}
              ...
              <p className="max-w-md font-normal text-white text-base md:text-lg text-center">
                Don't have an account? <span className="cursor-pointer italic" style={{ color: '#9DD2F2' }} onClick={() => navigate('/signup')}>Signup</span>
              </p>
            </motion.div>
          </motion.div>
        </motion.section>
